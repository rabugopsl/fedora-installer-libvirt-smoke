...
do
    ...
    if [[ -n $BOOTSTRAPIP ]]; then
        msg=$(ssh -oStrictHostKeyChecking=no core@${BOOTSTRAPIP} journalctl -n 1 -u bootkube -u tectonic 2> /dev/null)
        if [[ -n $(echo $msg | grep 'Tectonic installation is done') ]]; then
            echo ''
            if ! [[ -z $EXEC_CMD ]]; then
                bash -c "${EXEC_CMD}"
                exit 0
